...
.dataTables_length,
  ...
  .dataTables_filter input[type="search"] {
    ...
    /* Por ejemplo, ajusta el tama�o de la fuente, el color, etc. */
    padding: 6px;
    font-size: 14px;
    border: 1px solid #ccc;
    border-radius: 4px;
...
.dataTables_paginate {
  margin-top: 15px; /* Espacio entre la tabla y la paginaci�n */
}
...
<%
		...
		<%break;
			...
			<td>
			    <button type="submit" name="btnAutorizar" class="btn btn-success mr-3 mx-4" onclick="return confirm('�Est�s seguro de autorizar el prestamo solicitado?')">
            ...
            //Si la p�gina diferente a uno, si agrega el bot�n anterior.
               if(pag!=1){%>
                   <a href="ServletPrestamosxAutorizar?pag=<%=pag - 1%>">&lt;</a>
                      <%  }  //Calcula la cant de p�ginas a mostrar.
                            for (int i = 0; i < cantPag; i++) {
                         
                                if(i+1==pag){  //Si la p�gina es igual a la p�gina actual, muestra la etiqueta active.
                        %>
                            <span><%=i+1%></span>
                      
                      <%  } else { //Si no, sigue mostrando las etiquetas normales con la opci�n para desplazarse. %>
                        ...
                        if(pag!=cantPag){%>
                            <a href="ServletPrestamosxAutorizar?pag=<%=pag + 1%>">&gt;</a>
                <%} }  else { //Si el m�ximo de p�ginas no es mayor a 1, muestra solo una p�gina %>
                   			<span>1</span>
                <% }  %>						
	</div>
</div>	
            ...
            "columnDefs": [
                {
                    ...
                    "searchable": true, // Permite la b�squeda en estas columnas
